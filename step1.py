...
update_file['tcp.flags'] = update_file['tcp.flags'].apply(lambda x: int(str(x), 16) if '.' not in str(x) else 0)
update_file['ip.fragments'] = update_file['ip.fragments'].apply(lambda x: int(str(x), 16) if int(str(x), 16) <= sys.maxsize else 0)
